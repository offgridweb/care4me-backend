...
//		$pages = ProviderPage::where('provider',$provider->id)->count();
//		$provider->provider_listings = $pages;
//		$provider->save();
//	}

	$pages = ProviderPage::get();
	
	foreach($pages as $page){
		$spdate = $page->created_at;
		$page->sp_publish_date = $spdate;
		$page->save();
	}
	return;
	
	}
	public function xxadminresults(Request $request){
		try{
			
			$users_waiting = User::where('user_status',0)->where('created_at','>','2024-01-01 00:00:00')->get();
			$users_total = User::where('user_status',1)->get()->count();
			
			return array('waiting'=>$users_waiting,'active'=>0,'total'=>$users_total);
			
			
		}  catch (\Exception $e) {
				$message = substr($e,0,strpos($e,"Stack trace"));
				return array('message'=>$message,'error'=>true,'code'=>999);
		} 
		return 'TEST';	
	}
